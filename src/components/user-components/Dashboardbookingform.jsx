// src/components/user-components/DashboardBookingForm.jsx
...
const TABS = [
  { key: "delivery", label: "Delivery", sub: "Send or receive a package", icon: Truck },
  { key: "errand", label: "Errand", sub: "Grocery, bills, documents & more", icon: ShoppingBag },
  { key: "custom", label: "Custom", sub: "Describe your own request", icon: Sparkles },
];
  ...
  return (
    <div className="ud-field">
      <label className="ud-field-label">
        {label} {required && <span className="ud-req">*</span>}
      </label>
      {children}
    </div>
  );
...
function DeliveryForm({ data, setData }) {
  ...
  return (
    <>
      <h3 className="ud-form-title">Delivery Booking</h3>
      ...
      <Field label="Package Size" required>
        <div className="ud-pill-group">
          {PACKAGE_SIZES.map((p) => (
            <button
              key={p.key}
              type="button"
              className={`ud-pill ${data.size === p.key ? "selected" : ""}`}
        ...
        <div className="ud-pill-group">
          {URGENCY.map((u) => (
            <button
              key={u.key}
              type="button"
              className={`ud-pill ${data.urgency === u.key ? "selected" : ""}`}
      ...
      <div className="ud-field ud-field-full">
        <label className="ud-field-label">Delivery Notes (optional)</label>
        <textarea
...
function ErrandForm({ data, setData }) {
  ...
  return (
    <>
      <h3 className="ud-form-title">Errand Booking</h3>
      <p className="ud-form-desc">
        ...
        <div className="ud-task-grid">
          {TASK_TYPES.map((t) => (
            <button
              type="button"
              key={t.key}
              className={`ud-task-chip ${data.taskType === t.key ? "selected" : ""}`}
      ...
      <div className="ud-field ud-field-full">
        <label className="ud-field-label">Task Details</label>
        <textarea
          className="ud-textarea"
...
function CustomForm({ data, setData }) {
  ...
  return (
    <>
      <h3 className="ud-form-title">Customize Your Request</h3>
      <p className="ud-form-desc">
        ...
        <div className="ud-pill-group">
          {CONCIERGE_TIERS.map((t) => (
            <button
              key={t.key}
              type="button"
              className={`ud-pill ${data.tier === t.key ? "selected" : ""}`}
      ...
      <div className="ud-field ud-field-full">
        <label className="ud-field-label">
          Describe Your Request <span className="ud-req">*</span>
        </label>
        <textarea
...
export default function DashboardBookingForm() {
  ...
  return (
    <div className="ud-booking">
      <div className="ud-booking-tabs">
        {TABS.map((t) => (
          <button
            type="button"
            key={t.key}
            className={`ud-booking-tab ${tab === t.key ? "active" : ""}`}
            onClick={() => switchTab(t.key)}
          >
            <span className="ud-booking-tab-icon">
              <t.icon size={18} />
            </span>
            <span className="ud-booking-tab-text">
              <span className="ud-booking-tab-label">{t.label}</span>
              <span className="ud-booking-tab-sub">{t.sub}</span>
            </span>
          </button>
        ))}
      </div>
      ...
      {submitted ? (
        <div className="ud-booking-success">
        ...
        <div className="ud-booking-layout">
          ...
          <div className="ud-summary-panel">
            <h4>Order Summary</h4>
            ...
            <div className="ud-submit-stack">
              <button type="button" className="ud-btn-whatsapp" onClick={handleWhatsApp}>
                <IconWhatsapp size={18} />
                Confirm on WhatsApp
              </button>
              <button type="button" className="btn btn-outline-black ud-submit-block" onClick={handleEmail}>
                <Mail size={18} />
                Confirm by Email
              </button>
            </div>
